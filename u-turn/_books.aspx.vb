...
Partial Class _abooks
    ...
    Protected Sub rpBooks_ItemCommand(source As Object, e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rpBooks.ItemCommand
        Response.Redirect("_edit_book.aspx?id=" & e.CommandName & "&isrtl='ltr'")
    End Sub

    Protected Sub RadSearchBox1_Search(sender As Object, e As Telerik.Web.UI.SearchBoxEventArgs) Handles RadSearchBox1.Search
        ds_Books.SelectCommand = "select [id], SUBSTRING(title, 1, 20) as mytitle, title, author, publisher, price, image, category, isrtl from books WHERE [dir]='ltr' AND id=" & e.Value
    ...
    Protected Sub btnAdd_Click(sender As Object, e As System.EventArgs) Handles btnAdd.Click
        ...
        Using cn As New SqlConnection(cnString)
            Using cmd As New SqlCommand("SELECT [position] from [books] WHERE position=(SELECT MAX(position) FROM [books] WHERE [dir]='ltr' )", cn)
    ...
    Protected Sub btnGo_Click(sender As Object, e As System.EventArgs) Handles btnGo.Click
        ds_Books.SelectCommand = "select [id], SUBSTRING(title, 1, 20) as mytitle,title, author, publisher, price, image, category, isrtl from books WHERE [dir]='ltr' ORDER BY position ASC"
        rpBooks.DataBind()
